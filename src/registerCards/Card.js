...
export default class Card extends Component {
  ...
  exitEditMode() {
    ...
    let activeCard =
      this.state.cards.length > this.state.index
        ? this.state.cards[this.state.index]
        : null;

    if (
      activeCard &&
      activeCard.title.toUpperCase() === this.state.title.toUpperCase()
    ) {
      this.props.updateCard(
        this.props.cardId,
        Object.assign({}, activeCard, { title: this.state.title })
      );
    } else {
  ...
  getStateWithNewIndex(prevState, index) {
    ...
    if (
      newIndex < 0 ||
      newIndex >= Math.min(prevState.cards.length, this.maxCards)
    ) {
  ...
  renderCardTitle() {
    let title = this.state.title
      ? this.state.title
      : 'No title (click to edit)';
    let Title = styled.div`width: 100%;`;
    return (
      <Title onClick={this.enterEditMode}>
        {title}
      </Title>
    );
  }
    ...
    var cardAlternatives = activeCardAlternatives.map((card, cardIndex) =>
      <Alternative active={this.state.index === cardIndex} key={card.title}>
        <ClickAbleAlternative
          onTouchStart={this.handleCardClick(cardIndex)}
          onMouseDown={this.handleCardClick(cardIndex)}
        >
          {card.title}
        </ClickAbleAlternative>
      </Alternative>
  ...
  render() {
    ...
    var cardTitle = this.state.editing
      ? this.renderEditableCardTitle()
    ...
    return (
      <CardContainer>
        {cardTitle}
        <ButtonContainer>
          <DangerButton onClick={onDelete} center>
            Delete
          </DangerButton>
        </ButtonContainer>
      </CardContainer>
