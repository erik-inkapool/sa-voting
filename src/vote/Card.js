...
const SligtlySmallerText = styled.p`
    font-size: 0.8em;
`

const Card = ({card, updateVotes, deleteOption, updateOptionName, addOption}) => {
    ...
    return (
        <CardContainer>
            ...
            <SligtlySmallerText><EmphasisText>{elect ? elect : 'For'}:</EmphasisText> {resolutions.for}</SligtlySmallerText>
            {resolutions.against ? <SligtlySmallerText><EmphasisText>Against:</EmphasisText> {resolutions.against}</SligtlySmallerText> : ''}
            {
                _.filter(options, option => !option.deleted).map(option => <Option
                    key={option.id}
                    id={option.id}
                    name={option.name}
